...
pub fn draw(app: &mut App, terminal: &mut CrosstermTerminal)->Result<(), Box<(dyn Error)>>{
    ...
    terminal.draw(|f| {
        ...
        // render word counter
        f.render_widget(Paragraph::new(format!("{} correct",app.correct_words)).alignment(ratatui::layout::Alignment::Center), counter_layout[0]); // correct
        f.render_widget(Paragraph::new(format!("{} incorrect",app.incorrect_words)).alignment(ratatui::layout::Alignment::Center), counter_layout[1]); // incorrect
